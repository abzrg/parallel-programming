...
desc: Calculates pi by integrating f function using OpenMP.
      In this version, an atomic has been used.
...
int main(void)
{
    ...
    #pragma omp parallel num_threads(NUM_THREADS)
    {
        ...
        for (int i = id; i < (int)num_intervals; i += num_threads_local) {
            ...
            // No array here, so no false sharing.
            local_sum += f(x) * dx;
        }

        // Note that local_sum goes "out of scope" beyond the parallel region.
        // Atomic protects summation into pi so that updates don't conflict.
    #pragma omp atomic
        pi += local_sum; // Only a few simple operations are allowed here
    }
